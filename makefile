...
#FILES += server/drivers/eth/dummy.o
...
$(LINUX_TARGET): $(LINUX_FILES)
	$(CC) $(LINUX_OPTIONS) $(LINUX_FILES) $(LINUX_GENERATED) -o $(LINUX_TARGET)

$(TEMP_LINUX)/%.o: $(SRC)/%.c $(HEADERS)
	@mkdir -p "$(shell dirname "$@")"
	$(CC) $(LINUX_OPTIONS) -c $< -o $@

$(TEMP_LINUX)/%.g1.o: $(TEMP_LINUX)/%.g1.c $(TEMP_LINUX)/%.g1.h
	$(CC) $(LINUX_OPTIONS) -Wno-overlength-strings -c $< -o $@
...
2cstr:
	$(CC) -std=c11 $(SRC)/2cstr.c -o 2cstr
