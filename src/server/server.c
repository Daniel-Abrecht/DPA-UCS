...
const DPAUCS_interface_t* DPAUCS_getInterfaceByLogicAddress( const DPAUCS_logicAddress_t* logicAddress ){
  for(int i=0;i<MAX_LOGIC_ADDRESSES;i++){
    if( address_list[i].logicAddress && DPAUCS_compare_logicAddress(address_list[i].logicAddress,logicAddress) )
...
const DPAUCS_interface_t* DPAUCS_getInterfaceByPhysicalAddress( const DPAUCS_mac_t mac ){
  for( struct DPAUCS_ethernet_driver_list* driver = DPAUCS_ethernet_driver_list; driver; driver = driver->next )
    for( struct DPAUCS_interface_list* interface = driver->entry->interface_list; interface; interface = interface->next )
      if( !memcmp( interface->entry->mac, mac, sizeof(DPAUCS_mac_t) ) )
